                ...
                <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div class="relative bg-gradient-to-br {{ $cardTheme }} p-4 text-white sm:p-5">
                        ...
                        <div class="mb-4 flex items-center justify-between text-xs font-semibold uppercase tracking-widest text-blue-100">
                            ...
                            <span class="rounded-full bg-white/20 px-2.5 py-1">{{ $statusLabel }}</span>
                        </div>

                        <p class="text-sm font-medium text-blue-100 truncate">{{ $holderLabel }}</p>
                        <p class="card-digits mt-2 whitespace-nowrap text-[0.98rem] font-semibold leading-tight tracking-[0.08em] sm:text-[1.12rem] sm:tracking-[0.1em] lg:text-[1.2rem]">
                            {{ $displayNumber }}
                        </p>
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-2 text-[11px] sm:gap-3 sm:text-xs">
                            ...
                            <div>
                                ...
                                <p id="balanceValue" class="card-digits mt-1 text-[1.6rem] font-bold leading-none tracking-tight text-white sm:text-[1.9rem]">
                                    ${{ number_format((float) ($card->cardBalance ?? 0), 2) }}
                                </p>
                            </div>
                            <div>
                                <p class="text-blue-100">Card Type</p>
                                <p class="mt-1 font-semibold text-white">{{ ucfirst((string) ($card->type ?? '—')) }}</p>
                            </div>
                        </div>

                        <p class="mt-4 text-right text-xl font-extrabold italic tracking-wide sm:mt-5 sm:text-2xl">{{ $organization !== '' ? $organization : 'CARD' }}</p>
                    </div>
                    ...
                    <div class="space-y-2 p-4">
                        ...
                        <div class="flex items-center justify-between gap-3 text-sm">
                            ...
                            <span class="font-medium text-slate-900 truncate max-w-[11rem]" title="{{ $card->email }}">{{ $card->email ?? '—' }}</span>
                        </div>
                        <div class="pt-1">
                            <span class="inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold {{ $statusChipClass }}">
                                {{ $statusLabel }}
                            </span>
                        </div>
                    </div>
            ...
            <section class="lg:col-span-8 space-y-4">
                <div class="rounded-xl border border-blue-100 bg-blue-50 px-3 py-3 sm:px-4">
                    ...
                    <ul class="mt-2 list-disc space-y-1 pl-4 text-[11px] text-slate-600 sm:pl-5 sm:text-xs">
                        <li><span class="font-semibold">Shared access:</span> Anyone with this link can view card details and recent activity.</li>
                        <li><span class="font-semibold">Card safety:</span> Only share this page with recipients you trust.</li>
                        <li><span class="font-semibold">Status aware:</span> Card usage depends on current card status and merchant acceptance.</li>
                        <li><span class="font-semibold">Data freshness:</span> Use the reload icon near balance to refresh the latest values.</li>
                    </ul>
                </div>
                                    ...
                                    @foreach ($transactions as $tx)
                                        @php
                                            ...
                                            $txStatusClass = match (strtolower($txStatus)) {
                                                'complete', 'completed', 'success', 'successful', 'refunded' => 'bg-emerald-100 text-emerald-700',
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-3 py-2.5 sm:px-4 sm:py-3 text-right card-digits">${{ number_format(abs($amount), 2) }}</td>
                                            <td class="px-3 py-2.5 sm:px-4 sm:py-3">
                                                <span class="rounded-full px-2 py-1 text-xs font-medium {{ $txStatusClass }}">{{ $txStatus }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <footer class="mt-8 border-t border-slate-200 bg-white">
        <div class="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-5 text-[10px] text-slate-500 md:flex-row md:items-end md:justify-between">
            <div class="space-y-0.5">
                ...
                <p>{{ $card->email ?? 'support@lanocard.com' }}</p>
            </div>
