...
const mainBalanceEl = document.getElementById('mainBalance');
    ...
    var BKASH_RATE = 126; // $1 = 126 BDT
    var BKASH_DEPOSIT_FEE = 1; // $1 fixed deposit fee
    document.querySelectorAll('.add-manual-option').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        var panelPayoneer = document.getElementById('manualInstructionsPayoneer');
        var panelBkash = document.getElementById('manualInstructionsBkash');
        if (method === 'bkash') {
          panelPayoneer.classList.add('hidden');
          panelBkash.classList.remove('hidden');
          document.getElementById('bkashAmountBdt').value = '';
          document.getElementById('bkashTransactionId').value = '';
          document.getElementById('bkashEquivalentUsd').textContent = '$0.00';
          var receivedEl = document.getElementById('bkashReceivedUsd');
          if (receivedEl) receivedEl.textContent = '$0.00';
          document.getElementById('bkashFormAfterAmount').classList.remove('hidden');
          document.getElementById('bkashProcessingStatus').classList.add('hidden');
        } else {
    ...
    var bkashAmountBdt = document.getElementById('bkashAmountBdt');
    var bkashEquivalentUsd = document.getElementById('bkashEquivalentUsd');
    var bkashReceivedUsd = document.getElementById('bkashReceivedUsd');
    function updateBkashReceived() {
      var bdt = parseFloat(bkashAmountBdt.value || '0') || 0;
      var equivalentUsd = bdt / BKASH_RATE;
      var received = Math.max(0, equivalentUsd - BKASH_DEPOSIT_FEE);
    ...
    document.getElementById('bkashSubmitBtn')?.addEventListener('click', function() {
      var bdt = parseFloat(document.getElementById('bkashAmountBdt').value || '0') || 0;
      var txId = (document.getElementById('bkashTransactionId').value || '').trim();
      if (bdt < 252) {
        alert('Minimum amount is 252 BDT (≈ $2).');
        return;
      }
      if (!txId) {
      ...
      document.getElementById('bkashFormAfterAmount').classList.add('hidden');
      document.getElementById('bkashProcessingStatus').classList.remove('hidden');
      document.getElementById('bkashAmountBdt').value = '';
      document.getElementById('bkashTransactionId').value = '';
      if (bkashEquivalentUsd) bkashEquivalentUsd.textContent = '$0.00';
      if (bkashReceivedUsd) bkashReceivedUsd.textContent = '$0.00';
    });
